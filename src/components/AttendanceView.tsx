...
export default function AttendanceView({ getToken, participants }: Props) {
  ...
  const submitDirectScan = async (sessionId: string) => {
    setScanLoading(true);
    setScanError(null);
    try {
      const token = await getToken();
      const res = await fetch(`/api/attendance/${encodeURIComponent(sessionId)}/scan`, {
        method: 'POST',
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleScanCode = (rawCode: string) => {
    ...
    try {
      ...
      const upperTarget = targetSessionId.toUpperCase();
      ...
      if (isDailyAttendance) {
        ...
        // Absen Proker / Sesi Kegiatan: Scan QR Saja!
        submitDirectScan(targetSessionId);
      }
    } catch (err: any) {
      isScanProcessingRef.current = false;
  ...
  const submitSelfieAttendance = async () => {
    ...
    try {
      ...
      if (!isSuperAdmin && (!currentGps || typeof currentGps.lat !== 'number' || typeof currentGps.lng !== 'number')) {
        throw new Error('Lokasi GPS tidak terdeteksi. Silakan aktifkan Lokasi/GPS pada HP Anda dan berikan izin lokasi di browser.');
      }
      ...
      if (!isSuperAdmin && (!selfieDataUrl || !selfieDataUrl.startsWith('data:image'))) {
        throw new Error('Foto selfie belum terambil. Pastikan kamera sudah menyala dan wajah terlihat di layar.');
      }
    ...
    <div className="space-y-6" id="attendance-view-container">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-base leading-tight">
                    {scanStep === 'selfie' ? 'Foto Selfie Presensi Harian' : 'Scan Presensi Kehadiran'}
                  </h3>
                  <p className="text-[11px] text-emerald-100">
                    {scanStep === 'selfie' ? 'Wajib Selfie di Posko & memperlihatkan Banner Posko' : 'Scan QR Code Kegiatan, Check-In, atau Check-Out'}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-5">
              {scanSuccessResult ? (
                ...
                <div className="space-y-4 text-center animate-in fade-in duration-200">
                  ...
                  {/* Banner & Posko Selfie Instruction Box */}
                  ...
                  <div className="relative bg-black rounded-3xl overflow-hidden min-h-[280px] max-h-[340px] flex items-center justify-center border-2 border-emerald-500 shadow-inner">
                    ...
                    <div className="absolute inset-0 border-4 border-dashed border-white/60 rounded-full w-48 h-56 m-auto pointer-events-none shadow-2xl flex items-end justify-center pb-3">
                      <span className="bg-black/75 text-amber-300 text-[10px] font-bold px-2.5 py-1 rounded-full backdrop-blur-xs border border-amber-400/40 text-center">
                        📸 Wajah & Banner Posko
                      </span>
                    </div>
                  </div>
      ...
      {qrModalSession && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl w-full max-w-lg lg:max-w-4xl overflow-hidden shadow-2xl border border-gray-100 animate-in fade-in zoom-in-95 duration-200 my-auto">
            {/* Header */}
            <div className="p-5 bg-gradient-to-r from-gray-900 to-slate-800 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-300">Tampilkan ke Peserta untuk Discan</p>
                </div>
              </div>
              <button
            ...
            {/* Content: Grid Layout for Desktop (Left: QR Code, Right: Live Participant List) */}
            <div className="p-5 lg:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              
              {/* SISI KIRI: QR Code */}
              <div className="lg:col-span-6 flex flex-col items-center text-center space-y-4 bg-gray-50/70 p-5 rounded-2xl border border-gray-100">
                <div>
                ...
                <div className="bg-white p-5 rounded-3xl border-2 border-dashed border-emerald-200 inline-block mx-auto shadow-sm">
                  <QRCodeSVG
              ...
              {/* SISI KANAN: Live Status Scan QR & Participant List */}
              <div className="lg:col-span-6 flex flex-col space-y-3 h-full justify-between">
                <div className="flex items-center justify-between text-xs pb-1 border-b border-gray-100">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-500 hover:text-emerald-600 transition-colors cursor-pointer"
                      title="Perbarui data scan"
                    >
                      <RefreshCw className={`w-4 h-4 ${qrModalLoading ? 'animate-spin text-emerald-600' : ''}`} />
                    </button>
                  </div>
                </div>

                <div className="max-h-[320px] lg:max-h-[340px] overflow-y-auto bg-gray-50/80 p-3 rounded-2xl border border-gray-200/80 space-y-2 text-left text-xs shadow-inner">
                  {qrModalRecords.length === 0 ? (
                    <p className="text-gray-400 text-center py-8 italic text-xs">Memuat daftar peserta...</p>
                  ) : (
                    qrModalRecords.map((rec, i) => (
                      <div key={i} className="flex items-center justify-between py-2 px-3 bg-white rounded-xl border border-gray-100 shadow-2xs hover:border-emerald-100 transition-colors">
                        <span className="font-semibold text-gray-800 truncate pr-2 text-xs">{rec.name}</span>
                        {rec.status === 'Hadir' ? (
                          <span className="text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200 flex items-center gap-1 shrink-0">
                            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> Hadir {rec.checkInTime ? `(${rec.checkInTime})` : ''}
                          </span>
                        ) : (
                          <span className="text-[11px] font-bold text-gray-500 bg-gray-100 px-2.5 py-1 rounded-full flex items-center gap-1 shrink-0">
                            <Clock className="w-3.5 h-3.5 text-gray-400" /> {rec.status || 'Belum Absen'}
                          </span>
                        )}
                      </div>
                    ))
                ...
                <button
                  onClick={() => setQrModalSession(null)}
                  className="w-full py-3 bg-gray-900 hover:bg-black text-white font-bold text-sm rounded-xl transition-all shadow-md mt-auto cursor-pointer"
                >
                  Tutup Tampilan QR
                </button>
              </div>
